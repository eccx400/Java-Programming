...
public class Ch47ArrayAlgorithmsEx1_6
{
    ...
    public int[] Ex2_TwoLargest( int[] data )
    {
        ...
        for ( int index = 0; index < data.length; index++ )
        {
            // System.out.printf( "index: %d\t largest: %d\t large2: %d\n",
            // index, largest, large2 );
    ...
    public int[] Ex5_Smooth( int[] signal )
    {
        ...
        for ( int x = 1; x < smooth.length - 1; x++ )
        {
    ...
    public double[] Ex6_DataTweeker( String fileName ) throws IOException
    {
        ...
        double[] array = new double[countArray];
        ...
        {
            if ( Math.abs( array[count] - average ) > Math.abs( xfar
                - average ) )
            {
                xfar = array[count];
            }
        ...
        for ( int count = 0; count < countArray; count++ )
        {
            average2 += array[count];
        }

        average2 = ( average2 - xfar ) / ( array.length - 1 );
        scanner.close();

        return new double[] { average, xfar, average2 };
    }
