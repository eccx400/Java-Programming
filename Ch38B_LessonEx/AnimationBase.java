...
public class AnimationBase extends JApplet implements ActionListener
{
     ...
     * It is responsible for drawing one frame of the animation.
     * The frame is drawn to the graphics context g. The parameters
     * width and height give the size of the drawing area. drawFrame()
     * should begin by filling the drawing area with a background color
     * (as is done in this version of drawFrame). It should then draw
     * the contents of the frame. The routine can call getFrameNumber()
     * to determine which frame it is supposed to draw. It can call
     * getElapsedTime() to find out how long the animation has been
     * running, in milliseconds. The functions getWidth() and getHeight()
     * tell the applet's size. Note that this routine should not take
     * a long time to execute! This version of this routine simple
     * displays the frame number and elapsed time in each frame.
     */
    ...
    */
     ...
     * to zero. The default value of frameCount is -1, meaning that by
     ...
     * the actual display time will probably be a bit longer. The default
     * value of 100 is reasonable. A value of 50 will give smoother
     * animations on computers that are fast enough to draw than many
     * frames per second. The value of time must be 10 or greater. If a
     * value less than 10 is passed, it will be changed to 10. Do not put
     * too much faith in the this value. It is only an approximation. If
     * you want to know exactly how long the animation has been running,
     * use getElapsedTime() instead of getFrameNumber().
     * 
